...
@Jmthon.ar_cmd(
    pattern="تخزين المطـور(?:\s|$)([\s\S]*)",
    command=("تخزين المطـور", plugin_category),
    info={
        ...
        "usage": "{tr}تخزين المطـور <المعرف/الايدي>",
        "examples": "{tr}تخزين المطـور @catuserbot_support",
    },
)
...
async def _(event):  # sourcery no-metrics
    ...
    catevent = await edit_or_reply(
        event,
        f"<code>عدد الملفات وحجم الملف من </code>{_format.htmlmentionuser(userdata.first_name,userdata.id)}<code> فـي الـمطـور </code><b>{link}</b>\n<code>ربـما تأخـذ بعـض الوقـت تعـتمد عـلى عـدد رسـائل المـستـخدم</code>",
        parse_mode="HTML",
    )
